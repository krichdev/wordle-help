...
const StartsWithQ = ({ data }: Props) => {
   return (
    <div className="flex min-h-screen flex-col items-center justify-center py-2">
      <Head>
        <title>Wordle Words That Start with Q</title>
      ...
      <main className="flex w-full flex-1 flex-col items-center mt-20 px-20 text-center">
        <h1 className="text-6xl font-bold mb-8">
          {data.words.length} Wordle Words That Start With Q
        </h1>
        <Link href="/">
...
export async function getStaticProps() {
  ...
  const res = await fetch(`${path}/api/words?starts=q`);
